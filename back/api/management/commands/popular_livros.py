...
class Command(BaseCommand):
    ...
    @transaction.atomic
    def handle(self, *a, **o):
        df_editoras = pd.read_csv(o["arquivo_editoras"], encoding="latin-1")
        df_autores = pd.read_csv(o["arquivo_autores"], encoding="utf-8-sig")
        df_livros = pd.read_csv(o["arquivo_livros"], encoding="latin-1")
        ...
        # 2) Criação da coluna 'ID' em Autores
        ...
        # 4) Busca do autor de livros do dict e retornar o 'ID'
        ...
        # 3) Criação 'ID_EDITORAS' em Livros
             ...
             for r in df_livros.itertuples(index = False):
                _, created = Livro.objects.update_or_create(
                    ...
                    defaults = {
                        ...
                        "idioma": r.idioma or "",
                        "ano_publicacao": int(r.ano_publicacao) if pd.notna(r.ano) else None,
